/* Acolia 1.3 — Minijogo de dominó na chamada (só o paciente vê o botão).
   ...
   O robô sempre começa. Sem peça que encaixe, compra automaticamente até vir uma que sirva;
   acabou o monte (peças viradas na mesa), passa a vez.
...
(function () {
  ...
  function newGame() {
    ...
    g.turn = 1; // quem começa é sempre o robô
    return g;
  }
  ...
  function mount(stage, { onToggle } = {}) {
    ...
    el.innerHTML = `
      ...
      <div class="dm-row"><span class="dm-grp">🤖 Robô <span class="dm-backs" data-dm-robot></span></span>
        <span class="dm-grp">Monte <span class="dm-backs" data-dm-stock></span></span></div>
    ...
    let g = null;
    let timer = null;
    let note = ''; // recado do momento (ex.: "Comprando…")
    const backs = (n) => `${'<i></i>'.repeat(Math.min(n, 21))} <small>${n}</small>`;

    function render() {
      $('[data-dm-score]').textContent = `Você ${score[0]} × ${score[1]} Robô`;
      $('[data-dm-robot]').innerHTML = backs(g.hands[1].length);
      $('[data-dm-stock]').innerHTML = backs(g.stock.length);
      const e = ends(g);
      const board = $('[data-dm-board]');
      board.innerHTML = g.chain.length
        ? g.chain.map((t, k) => tileHtml(t[0], t[1], k === 0 || k === g.chain.length - 1 ? 'end' : '')).join('')
        : '<span class="dm-empty">A mesa está vazia</span>';
      board.scrollTop = board.scrollHeight;
      const mine = g.turn === 0 && !g.over && !g.busy;
      const ms = mine ? movesOf(g, 0) : [];
      // Vez do robô: peças cinza e travadas. Sua vez: brancas; as que encaixam ficam destacadas.
      const hand = $('[data-dm-hand]');
      hand.classList.toggle('locked', !mine);
      hand.innerHTML = g.hands[0].map((t, i) => {
      ...
      $('[data-dm-sides]').classList.toggle('hidden', g.pending == null || !mine);
      const st = $('[data-dm-status]');
      st.classList.toggle('you', mine);
      st.textContent = g.over ? '' : note || (g.turn === 1 ? '🤖 Robô pensando…'
        : e ? `Sua vez! Pontas: ${e[0]} e ${e[1]}` : 'Sua vez!');
    ...
    // Sua vez: se nenhuma peça encaixa, compra sozinho (uma por vez) até vir uma que sirva;
    // acabou o monte, passa a vez
    function yourTurn() {
      g.turn = 0;
      if (movesOf(g, 0).length) { note = ''; g.busy = false; return render(); }
      g.busy = true;
      const step = () => {
        if (!g || g.over) return;
        if (movesOf(g, 0).length) { g.busy = false; note = 'Comprou até vir uma peça que serve. Sua vez!'; return render(); }
        if (g.stock.length) {
          g.hands[0].push(g.stock.pop());
          note = 'Você não tem peça que encaixe: comprando do monte…';
          render();
          timer = setTimeout(step, 550);
          return;
        }
        note = 'Acabou o monte e nenhuma peça encaixa: você passa a vez.';
        render();
        timer = setTimeout(() => {
          g.busy = false;
          note = '';
          pass(g);
          if (!finishIfOver()) robotTurn();
        }, 1400);
      };
      note = 'Você não tem peça que encaixe: comprando do monte…';
      render();
    ...
    function robotTurn() {
      ...
      timer = setTimeout(() => {
        ...
        let m = robotMove(g);
        let bought = 0;
        while (!m && g.stock.length) { g.hands[1].push(g.stock.pop()); bought += 1; m = robotMove(g); }
        if (m) play(g, 1, m); else pass(g);
        if (finishIfOver()) return;
        yourTurn();
        if (!g.busy) note = bought ? `O robô comprou ${bought} peça${bought > 1 ? 's' : ''}. Sua vez!` : m ? '' : 'O robô passou a vez. Sua vez!';
        render();
      }, 800 + Math.random() * 700);
    }
    ...
    el.addEventListener('click', (ev) => {
      ...
      if (side && g.pending != null) humanPlay({ i: g.pending, side: side.dataset.dmSide });
    });
    ...
    function open() {
      ...
      if (!g) start();
      else if (!g.over) { if (g.turn === 1) robotTurn(); else yourTurn(); } // continua de onde parou
      else render();
      onToggle?.(true);
    }
    function close() {
