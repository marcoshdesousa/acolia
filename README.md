...
- **Meus pacientes** (painel do profissional → Atendimento): lista de quem **já fez consulta** com ele pela Acolia (chamada que chegou a começar, ligada a uma conversa), com nome completo, CPF, nascimento, município, número de consultas e data da última. **Filtro** pelo nome ou pelo CPF (com ou sem pontos). Botões **Baixar PDF** (tabela com a logo da Acolia, nome e registro do profissional, páginas numeradas e aviso de dado confidencial/LGPD — gerado no servidor, sem biblioteca, `server/pdfTable.js`) e **Baixar planilha (Excel)**; os dois respeitam o filtro. Cada profissional vê só os próprios pacientes; paciente excluído não aparece.
