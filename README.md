...
6. **Sem janelinha flutuante**: a pessoa fica na tela da chamada. Embaixo da logo aparece um aviso pequeno, "Não saia desta tela nem atualize a página", e ao tentar atualizar ou fechar a página o navegador pergunta antes. Se mesmo assim sair do site, a câmera desliga (a outra pessoa vê a foto) e o áudio continua; ao voltar, a câmera religa sozinha.
   - **Minijogo (versão 1.3) — dominó, só para o paciente**: botão com o ícone de dominó na barra da chamada. O paciente joga contra o robô (dificuldade média: na maioria das vezes escolhe uma boa jogada, às vezes erra) enquanto continua vendo e ouvindo o profissional e se vendo. No computador a tela divide (vídeo à esquerda, jogo à direita); no celular o profissional fica em cima e o jogo embaixo, sem cobrir o rosto dele. Regras: 28 peças, 7 para cada, começa quem tem o maior duplo, sem peça que encaixe compra (acabou o monte, passa); ganha quem bater ou, se trancar, quem tiver menos pontos. Placar da chamada, "Jogar de novo" e ✕ para fechar (a partida continua de onde parou ao abrir de novo). O profissional não tem o botão. **Para tirar:** apagar `public/js/domino.js`, o `<script>` e o botão `data-game` em `atendimento.html`, a função `setupGame` e a linha `S.game?.close()` em `call.js` e o bloco "Minijogo (1.3)" do `app.css`.
