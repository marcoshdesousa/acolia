...
6. **Sem janelinha flutuante**: a pessoa fica na tela da chamada. Embaixo da logo aparece um aviso pequeno, "Não saia desta tela nem atualize a página", e ao tentar atualizar ou fechar a página o navegador pergunta antes. Se mesmo assim sair do site, a câmera desliga (a outra pessoa vê a foto) e o áudio continua; ao voltar, a câmera religa sozinha.
   - **Minijogo (versão 1.3) — dominó, só para o paciente**: botão com o ícone de dominó na barra da chamada. O paciente joga contra o robô (dificuldade média: na maioria das vezes escolhe uma boa jogada, às vezes erra) enquanto continua vendo e ouvindo o profissional e se vendo. No computador a tela divide (vídeo à esquerda, jogo à direita); no celular o vídeo continua inteiro e nítido (sem desfoque e sem fundo escuro) e o jogo fica na parte de baixo, só com as peças e rótulos pequenos por cima do vídeo. Regras: 28 peças, 7 para cada, as outras ficam viradas na mesa (monte); **o robô sempre começa**; sem peça que encaixe, **compra automaticamente** até vir uma que sirva (acabou o monte, passa a vez sozinho); ganha quem bater ou, se trancar, quem tiver menos pontos. Na vez do robô as peças do paciente ficam cinza e travadas; na vez dele ficam brancas, com "Sua vez!". Peças separadas, com a barra do meio marcada (inclusive o lado limpo). Placar da chamada, "Jogar de novo" / "Fechar" no fim e ✕ para fechar (a partida continua de onde parou ao abrir de novo). O profissional não tem o botão. **Para tirar:** apagar `public/js/domino.js`, o `<script>` e o botão `data-game` em `atendimento.html`, a função `setupGame` e a linha `S.game?.close()` em `call.js` e o bloco "Minijogo (1.3)" do `app.css`.
...
- **Meus pacientes** (painel do profissional → Atendimento): lista de quem **já fez consulta** com ele pela Acolia (chamada que chegou a começar, ligada a uma conversa), com nome completo, CPF, nascimento, município, número de consultas e data da última. **Filtro** pelo nome ou pelo CPF (com ou sem pontos) e pelo **período** das consultas ("De" e "Até", com atalhos Este mês, Mês passado, Este ano e Tudo; datas no horário de Brasília). Mostra o **resumo do período**: total de pacientes e de consultas (as colunas contam só as consultas do período). Botões **Baixar PDF** (tabela com a logo da Acolia, nome e registro do profissional, páginas numeradas período e totais, e aviso de dado confidencial/LGPD — gerado no servidor, sem biblioteca, `server/pdfTable.js`; **sem limite de páginas** — 10 mil pacientes = 455 páginas em menos de 1 s — com o **total de pacientes e consultas no fim**; o PDF é montado na memória e enviado direto, **não fica salvo no servidor**: se a pessoa não salvar, some, e pode gerar de novo quando quiser) e **Baixar planilha (Excel)**, com o período e os totais no final; os dois respeitam os filtros. Cada profissional vê só os próprios pacientes; paciente excluído não aparece.
